...
local SetItlStyle = function(eventAf)
  ...
  eventAf:GetChild("HeaderBackground"):diffusetopedge(color("0.3,0.3,0.3,1")):diffusebottomedge(color(
  "0.157,0.157,0.165,1"))
...
local BannerAndSong = function(x, y, zoom)
  ...
  af[#af + 1] = LoadFont(ThemePrefs.Get("ThemeFont") .. " Normal") .. {
    Name = "SongName",
    InitCommand = function(self)
      local songtitle = (GAMESTATE:IsCourseMode() and GAMESTATE:GetCurrentCourse():GetDisplayFullTitle()) or
      GAMESTATE:GetCurrentSong():GetDisplayFullTitle()
      if songtitle then self:settext(songtitle):zoom(2):maxwidth(500):vertalign("top"):y(90):diffuse(color(
        "1,0.972,0.792,1")) end
    end
  }
